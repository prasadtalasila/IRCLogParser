...
import util
import config
def createNickChangesGraph(log_dict,channel):
    ...
    Args:
        ...
        channel(str)  : single channel to perform analysis eg '#kubuntu-devel'
	...
	for day_content_all_channels in log_dict.values():		
		for day_content in day_content_all_channels:			
				...
				if(day_content["auxiliary_data"]["channel"] == channel):
						...
						if(line[0] =='=' and "changed the topic of" not in line):  #excluding the condition when user changes the topic. Search for only nick changes
							...
							while earlier_line_no >= 0: #to find the line just before "=="" so as to find time of Nick Change
								...
								if(day_log[earlier_line_no][0] != '='):								
									graph_nickchanges.add_edge(nick1,nick2,weight=day_log[earlier_line_no][1:6])
									...
									aggregate_nick_change_graph.add_edge(nick1,nick2,weight = year+"/"+month+"/"+ day +" - "+day_log[earlier_line_no][1:6])
									break

							if(earlier_line_no == -1):
								graph_nickchanges.add_edge(nick1,nick2,weight=rem_time)                                              
					...
					while(count >= 0):
						if(day_log[count][0]!='='):
							...
							break
						count=count-1
					daily_nickChange_list.append(graph_nickchanges)	
						

	if config.DAY_BY_DAY_ANALYSIS:
		return daily_nickChange_list
	else:
