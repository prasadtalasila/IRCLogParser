...
class NetworkTest(unittest.TestCase):
    ...
    def test_message_number_graph(self, mock_get_nick_sen_rec, mock_rec_list_splice, mock_correctLastCharCR, \
        ...
        to_graph_ret = util.load_from_disk(self.current_directory + "/data/message_number_graph/to_graph")
        ...
        mock_rec_list_splice.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_number_graph/rec_list_splice")
        mock_create_connected_nick_list.return_value = util.load_from_disk(
            self.current_directory + "/data/message_number_graph/conn_comp_list")
        mock_check_if_msg_line.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_number_graph/check_if_msg_line")
        mock_correctLastCharCR.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_number_graph/correctLastCharCR")
        mock_get_nick_sen_rec.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_number_graph/get_nick_sen_rec")
    ...
    def test_channel_user_presence_graph_and_csv(self):
        nicks = util.load_from_disk(self.current_directory + '/data/test_presence_nicks')
        nick_same_list = util.load_from_disk(self.current_directory + '/data/test_presence_nick_same_list')
        channels_for_user = util.load_from_disk(self.current_directory + '/data/test_presence_channels_for_user')
        nick_channel_dict = util.load_from_disk(self.current_directory + '/data/test_presence_nick_channel_dict')
        nicks_hash = util.load_from_disk(self.current_directory + '/data/test_presence_nicks_hash')
        channels_hash = util.load_from_disk(self.current_directory + '/data/test_presence_channels_hash')

        expected_dict_out = util.load_from_disk(self.current_directory + '/data/dict_out')
        expected_graph = util.load_from_disk(self.current_directory + '/data/graph')
    ...
    def test_filter_edge_list(self):
        capturedOutput = StringIO.StringIO()
        sys.stdout = capturedOutput
        network.filter_edge_list(self.current_directory + '/data/filter_edge_list_input.txt', 10000, 100)
        sys.stdout = sys.__stdout__
        output = capturedOutput.getvalue()
        f = open(self.current_directory + '/data/filter_edge_list_out.txt', 'r')
    ...
    def test_degree_analysis_on_graph(self):
        directed_graph = util.load_from_disk(self.current_directory + '/data/directed_graph')
        undirected_graph = util.load_from_disk(self.current_directory + '/data/undirected_graph')
        directed_deg_analysis_expected = util.load_from_disk(
            self.current_directory + '/data/directed_deg_analysis_result')
        undirected_deg_analysis_expected = util.load_from_disk(
            self.current_directory + '/data/undirected_deg_anlaysis_result')
    ...
    def test_message_time_graph(self, mock_get_nick_sen_rec, mock_correct_last_char_list, \
                                ...
                                mock_to_graph):
        to_graph_ret = util.load_from_disk(self.current_directory + "/data/message_time_graph/to_graph")

        conn_list = list(connected_components(to_graph_ret))

        mock_to_graph.return_value = to_graph_ret
        mock_rec_list_splice.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_time_graph/rec_list_splice")
        mock_create_connected_nick_list.return_value = util.load_from_disk(
            self.current_directory + "/data/message_time_graph/conn_comp_list")
        mock_check_if_msg_line.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_time_graph/check_if_msg_line")
        mock_get_nick_sen_rec.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_time_graph/get_nick_sen_rec")
        mock_correct_last_char_list.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_time_graph/correct_last_char_list")
    ...
    def test_message_number_bins_csv(self, mock_rec_list, mock_correct_last_char_list, mock_correct_last_char_CR):
        mock_correct_last_char_CR.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_number_bins_csv/correctLastCharCR")
        mock_correct_last_char_list.side_effect = util.load_from_disk(
            self.current_directory + "/data/message_number_bins_csv/correct_last_char_list")
